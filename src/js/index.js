...
import { measPoint, measDistance, measHeight, measAngle, measArea, measClear } from './measure';
...
function checkMovement (scene) {
  ...
  if (scene.lookAtPtNum !== null && scene.dofilterimages && !scene.cameraplaneview) {
    ...
    if (
      ...
      scene.filterImages(scene.camPix, scene.camFocal);
...
$('.prevImage').on('click', function (e) {
  ...
  if (scene.dofilterimages) {
    while (scene.frustrum[scene.currentid].isFiltered) {
      scene.currentid--;
      count++;
      if (scene.currentid < 0) {
        scene.currentid = scene.ncams - 1;
      }
...
$('#pointBudget').on('change', function (e) {
  const pointBudget = $(this).val();
  console.log('change point budget to %i', pointBudget);
  viewer.setPointBudget(pointBudget);
});

$('#pointSize').on('change', function (e) {
  const minPointSize = $(this).val();
  console.log('change node size to %i', minPointSize);
  viewer.minNodeSize = minPointSize;
});
...
// ======== MEASUREMENTS ==================
$('.measPoint').on('click', scene, measPoint);
$('.measDistance').on('click', scene, measDistance);
$('.measHeight').on('click', scene, measHeight);
$('.measAngle').on('click', scene, measAngle);
$('.measArea').on('click', scene, measArea);
$('#measClear').on('click', scene, measClear);
/*
...
*/
$('.btnColormap').on('click', changeColormap);
...
document.addEventListener('onMeasureTool', function (e) {
  console.log('onMeasureTool()');
  $('#navigationMode').html(e.detail.content);
});

function changeColormap (e) {
  console.log('changeColormap()', e, Potree.Gradients);

  let cmap = Potree.Gradients.PLASMA;

  switch (e.target.id) {
    case 'colormapGreyscale':
